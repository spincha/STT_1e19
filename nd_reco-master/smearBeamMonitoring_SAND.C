...
TH3 *herr_nu_E_iregion_ecal22;

TH1 *hEnterSTT_par;
TH3 *herr_nu_E_iregion_ecal3;
TH3 *herr_nu_E_iregion_ecal4;
TH3 *herr_nu_E_iregion_ecal5;
TH1 *hratio_backEcalE2totEcalE;
...
bool inFV_Meniscus(const TVector3 &pos){
  double x=pos.X();
  double y=pos.Y();
  double z=pos.Z();
  if(inFV(x,y,z)) return true;
  if(inLiqArMeniscus(pos)) return true;
  return false;
...
double getECAL_calE(double &totDe){
...
void get1ecalhit_info(int ihit, double &de_pass){
  ...
  const TG4HitSegment& h = event->SegmentDetectors[ECALNAME].at(ihit);
    ...
    */
    ...
    double dis2vtx=(h.Start.Vect()-event->Primaries.begin()->GetPosition().Vect()).Mag();
    //    if(dis2vtx>ecalhit2vtx_max) continue;
    if(iECALRegion==0 && dis2vtx>2500) return;
    else if(iECALRegion==1 && dis2vtx>2000) return;
    else if(iECALRegion>1 && dis2vtx>1200) return;
...
*/
bool getP_enteringSTT(int trackid, TVector3 &P, double &T){

  if(inSTT(event->Trajectories[trackid].Points[0].Position.Vect())) { /*std::cout<<" this track starts in STT !!!!! could be secondary particles , check !!!!!!!"<<std::endl; */ 
    P=event->Trajectories[trackid].Points[0].Momentum; 
    T=event->Trajectories[trackid].Points[0].Position.T();
    return true;}
  if(debug>=3) { 
  ...
  for(int i=1;i<event->Trajectories[trackid].Points.size();i++){
    ...
    if(inSTT(event->Trajectories[trackid].Points[i].Position.Vect())) { P=event->Trajectories[trackid].Points[i].Momentum; T=event->Trajectories[trackid].Points[0].Position.T(); return true;}    
  }
  return false;

}

bool getP_enteringSTT_fromMeniscus(int trackid, TVector3 &P){

  if(inSTT_notMeniscus(event->Trajectories[trackid].Points[0].Position.Vect())) { /*std::cout<<" this track starts in STT !!!!! could be secondary particles , check !!!!!!!"<<std::endl; */ 
    P=event->Trajectories[trackid].Points[0].Momentum; 
    //    T=event->Trajectories[trackid].Points[0].Position.T();
    return true;}
  ...
  for(int i=1;i<event->Trajectories[trackid].Points.size();i++){
    ...
    if(inSTT_notMeniscus(event->Trajectories[trackid].Points[i].Position.Vect())) { P=event->Trajectories[trackid].Points[i].Momentum; return true;}
  }
  return false;
...
bool smearChargedPar_stt(int trackid,TVector3 trueP, TVector3 &recoP, double &T){
  ...
  */
  ...
  double thetaYZ=TMath::ATan2(trueP.Y(),trueP.Z());
...
void smearEvent_STT(){
  ...
  //  if(!inFV_Meniscus(vtx.Vect())) return;
...
bool getBackEcalE(int trackid, double &backEcalE, double sttEnteringT){

  int n=event->Trajectories[trackid].Points.size();
  if(n<1) return false;
  ...
  backEcalE=totde;
  if(backEcalE==0) return false;
  return true;
  
}

int getCode(std::string name){
  ...
  if(name=="mu+" || name=="mu-")
    ...
    std::cout<<" check this name:"<<name<<std::endl;
    namecode=6;
  }
...
void smearEvent_ECAL(){
  ...
  double totDe;
  double ecalTotE=getECAL_calE(totDe);
  
  //  std::map<int,double> goodTrack_Ts;
  double tot_backecalE=0;
  ...
  for(auto id_hitpair: sttMap){
    ...
    double enteringT;
    if(fDetectorType==1)
      enter=getP_enteringSTT_fromMeniscus(trackid, pEnteringSTT);
    else
      enter=getP_enteringSTT(trackid, pEnteringSTT, enteringT);
    if(!enter) continue;
    ...
    double backEcalE=0;
    bool ifBackEcal=getBackEcalE(trackid, backEcalE, enteringT);

    if(ifBackEcal) tot_backecalE+=backEcalE;
    //    goodTrack_Ts[trackid]=T;
    double m=dbpdg->GetParticle(pdg)->Mass()*1000;  // MeV
    double m2=m;
    if(name!="mu+" && name!="mu-" && name!="proton" && name!="anti_proton" && name!="e+" && name!="e-")
      m2=dbpdg->GetParticle(211)->Mass()*1000;
    double E=sqrt(reco_pEnteringSTT.Mag2()+m*m);
    double KE=(E>m)?(E-m):0;
    
    double E2=sqrt(reco_pEnteringSTT.Mag2()+m2*m2);
    double KE2=(E2>m2)?(E2-m2):0;
    sttKE+=KE;
    sttKE2+=KE2;
    //    sttE+=E;
    if(debug>=1) std::cout<<"will add E/KE to nu: E:"<<E<<" KE:"<<KE<<" name:"<<name<<std::endl;

    int parcode=getCode(name);
    
    hEnterSTT_par->Fill(parcode);
    if(debug>=1) {if(name=="proton") std::cout<<"have proton :iEntry:"<<iEntry<<std::endl;}
  ...
  int planeID;
  if(fDetectorType==2){
    planeID=getECAL_Plane(event->Primaries.begin()->GetPosition().Vect());
    if(planeID>10) { std::cout<<"something wrong check the plane return, check check"<<std::endl; return;}
    nuE22=nuE2*(1-offsets[planeID]);
  }
  ...
  brRecoNuP4[3]=nuE22;
  brNPar=iFillPar;
  fNeutrinoEnergyRec=nuE22;
  tree->Fill();
  
  // int iECALRegioin=inECALRegion(event->Primaries.begin()->GetPosition().Vect());
  // if(tot_backecalE/ecalTotE>0.1) std::cout<<"------->iEntry:"<<iEntry<<std::endl;
  // hratio_backEcalE2totEcalE->Fill(tot_backecalE/ecalTotE);

  // double nuE4=nuE2-tot_backecalE;
  // double nuE5;
  // if(fDetectorType==2){
  //   //    planeID=getECAL_Plane(event->Primaries.begin()->GetPosition().Vect());
  //   if(planeID>10) { std::cout<<"something wrong check the plane return, check check"<<std::endl; return;}
  //   nuE5=nuE4*(1-offsets2[planeID]);
  // }

  // if(fDetectorType==2){
  //   herr_nu_E_iregion_ecal2->Fill((nuE2-brTrueNuP4[3])/brTrueNuP4[3], iECALRegioin, planeID);
  //   herr_nu_E_iregion_ecal22->Fill((nuE22-brTrueNuP4[3])/brTrueNuP4[3], iECALRegioin, planeID);
  //   herr_nu_E_iregion_ecal3->Fill((nuE3-brTrueNuP4[3])/brTrueNuP4[3], iECALRegioin, planeID);
  //   herr_nu_E_iregion_ecal4->Fill((nuE4-brTrueNuP4[3])/brTrueNuP4[3], iECALRegioin, planeID);
  //   herr_nu_E_iregion_ecal5->Fill((nuE5-brTrueNuP4[3])/brTrueNuP4[3], iECALRegioin, planeID);
...
int main(int argc, char** argv){
  ...
  */
  ...
  herr_nu_E_iregion_ecal22=new TH3F("herr_nu_E_iregion_ecal22","",100,-1,1,4,0,4,6,0,6);
  
  hEnterSTT_par=new TH1I("hEnterSTT_par","",10,0,10);
  herr_nu_E_iregion_ecal3=new TH3F("herr_nu_E_iregion_ecal3","",100,-1,1,4,0,4,6,0,6);
  herr_nu_E_iregion_ecal4=new TH3F("herr_nu_E_iregion_ecal4","",100,-1,1,4,0,4,6,0,6);
  herr_nu_E_iregion_ecal5=new TH3F("herr_nu_E_iregion_ecal5","",100,-1,1,4,0,4,6,0,6);
  hratio_backEcalE2totEcalE=new TH1F("hratio_backEcalE2totEcalE","",100,0,1);
  ...
  for(int i=startEntry;i<endplusEntry;i++){
    ...
    if(inSTT(event->Primaries.begin()->GetPosition().Vect())) {
      if(inLiqArMeniscus(event->Primaries.begin()->GetPosition().Vect()))
	fDetectorType=1;
      else
	fDetectorType=0;      
    }
    else fDetectorType=2;
  ...
  herr_nu_E_iregion_ecal22->Write();
  //  outTreeF->Write();

  hEnterSTT_par->Write();
  herr_nu_E_iregion_ecal3->Write();
  herr_nu_E_iregion_ecal4->Write();
  herr_nu_E_iregion_ecal5->Write();
  hratio_backEcalE2totEcalE->Write();
  for(int i=0;i<4;i++){
    hvtxXY[i]->Write();
    hvtxZY[i]->Write();
  }
